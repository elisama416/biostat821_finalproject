...
def test_compare_sequences():
    ...
    expected_differences = [(2, '-', 'C'), (4, 'T', '-'), (6, 'A', 'G'), (8, 'A', 'U')]
    assert differences == expected_differences, f"Expected {expected_differences}, got {differences}"
...
def test_report_variants_no_variants():
    ...
    differences = []
    assert report_variants(differences) == "No variants found.", "Report should indicate no variants found"
...
def test_report_variants_with_variants():
    ...
    assert report_variants(differences) == expected_report, f"Expected {expected_report}, got {report_variants(differences)}"

def test_align_sequences_different_lengths():
    """Test the alignment of sequences of different lengths."""
    seq1 = "GATTACA"
    seq2 = "GCAT"
    aligned1, aligned2 = align_sequences(seq1, seq2)
    expected_aligned1 = "G-ATTACA"
    expected_aligned2 = "GCA-T---"
    assert aligned1 == expected_aligned1, f"Expected '{expected_aligned1}', got '{aligned1}'"
    assert aligned2 == expected_aligned2, f"Expected '{expected_aligned2}', got '{aligned2}'"
